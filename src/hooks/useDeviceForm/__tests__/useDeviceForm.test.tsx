...
describe('useDeviceForm', () => {
    ...
    it('should update form state on handleChange', () => {
        const { result } = renderHook(() => useDeviceForm('1'));
        act(() => {
            result.current.handleChange({ target: { name: 'system_name', value: 'Updated Device', addEventListener: jest.fn(), removeEventListener: jest.fn(), dispatchEvent: jest.fn() } } as unknown as React.ChangeEvent<HTMLInputElement>);
        });
    ...
    it('should call updateDevice on handleSubmit if deviceId is provided', async () => {
        ...
        (useDeviceUpdate as jest.Mock).mockReturnValue({ mutateAsync: updateDevice });
        const { result } = renderHook(() => useDeviceForm('1'));
        await act(async () => {
    ...
    it('should call createDevice on handleSubmit if deviceId is not provided', async () => {
        ...
        (useCreateDevice as jest.Mock).mockReturnValue({ mutateAsync: createDevice });
    ...
    it('should return error if required fields are missing on handleSubmit', async () => {
        const { result } = renderHook(() => useDeviceForm());
        act(() => {
            result.current.handleChange({ target: { name: 'system_name', value: '', addEventListener: jest.fn(), removeEventListener: jest.fn(), dispatchEvent: jest.fn() } } as unknown as React.ChangeEvent<HTMLInputElement>);
        });
        const response = await result.current.handleSubmit();
        expect(response).toEqual({ status: 'error', message: 'Please fill in all required fields.' });
    });
});
