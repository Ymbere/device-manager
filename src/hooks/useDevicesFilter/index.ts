...
export const useDeviceFilters = (devices: Device[]) => {
  ...
  const handleChangeDeviceTypeFilter = React.useCallback((selectedValues: DropdownItem | DropdownItem[]) => {
  ...
  const handleChangeDeviceSortingFilter = React.useCallback((selectedValue: DropdownItem | DropdownItem[]) => {
  ...
  const filteredDevicesList = React.useMemo(() => {
